<?php

class Session {
    
    private $admin_id;
    private $user_id;
    public $email;
    public $mobile_number;
    public $username;
    private $last_login;
    private $last_user;
    public const MAX_LOGIN_AGE = 60*60*24; // 1 day
    public function __construct() {
         session_start(); // turn on sessions if needed

        $this->check_stored_login();
        $this->user_stored_login();
    }
    public function login($admin) {
        if($admin){
            // prevent session fixation attacks
            session_regenerate_id();
           $this->admin_id = $_SESSION['admin_id'] = $admin->id;
           $this->username = $_SESSION['username'] = $admin->username;
            $this->last_login = $_SESSION['last_login'] = time();
        }
        
        return true;
    }

    public function log_user($user) {
        if($user){
            // prevent session fixation attacks
            session_regenerate_id();
           $this->user_id = $_SESSION['user_id'] = $user->id;
         $this->email = $_SESSION['email'] = $user->email;
         $this->mobile_number = $_SESSION['mobile_number'] = $user->phone_number;
            $this->last_user = $_SESSION['last_user'] = time();
        }
        
        return true;
    }

    
    public function is_logged_in(){
       //return isset($this->admin_id);
        return isset($this->admin_id) && $this->last_logged_is_recent();
    }

    public function is_user_in(){
     
        //return isset($this->admin_id);
         return isset($this->user_id) && $this->last_user_is_recent();
     }
    
    public function logout() {
        unset($_SESSION['admin_id']);
          unset($_SESSION['username']);
          unset($_SESSION['last_login']);
        unset($this->admin_id);
           unset($this->username);
           unset($this->last_login);
        return true;
    }

    public function user_logout() {
        unset($_SESSION['user_id']);
          unset($_SESSION['email']);
          unset($_SESSION['mobile_number']);
          unset($_SESSION['last_user']);
        unset($this->user_id);
           unset($this->email);
           unset($this->mobile_number);
           unset($this->last_user);
        return true;
    }


    
    private function check_stored_login() {
        if(isset($_SESSION['admin_id'])) {
            $this->admin_id = $_SESSION['admin_id'];
            $this->username = $_SESSION['username'];
            $this->last_login = $_SESSION['last_login'];
        }
    }
    private function user_stored_login() {
     if(isset($_SESSION['user_id'])) {
            $this->user_id = $_SESSION['user_id'];
            $this->email = $_SESSION['email'];
            $this->phone_number = $_SESSION['mobile_number'];
            $this->last_user = $_SESSION['last_user'];
        }
    }
    
    private function last_logged_is_recent() {
        if(!isset($this->last_login)) {
            return false;
        }
        elseif(($this->last_login + self::MAX_LOGIN_AGE) < time()) {
            return false;
        }
        else {
            return true;
        }
    }


    private function last_user_is_recent() {
        if(!isset($this->last_user)) {
            return false;
        }
        elseif(($this->last_user + self::MAX_LOGIN_AGE) < time()) {
            return false;
        }
        else {
            return true;
        }
    }
    
    public function message($msg="") {
        if(!empty($msg)) {
            // THen this is a set message
            $_SESSION['message'] = $msg;
            return true;
        }
        else {
            // Then this is a get message
            return $_SESSION['message'] ?? '';
        }
    }
    
    public function clear_message() {
           unset($_SESSION['message']);
    }
}